...
/* Function for rendering an image */
void mgui_render_canvas_img(JsonPair kv, JsonObject root){
  /* Create LVGL object */
  lv_obj_t * canvas_img = lv_img_create(lv_scr_act());
  ...
  /*
  Everything below is an attempt at getting an image to render. At the moment it renders only the top part of the image. 
  Something is probably wrong with the decoding. 
  Don't know if it's possible to do it this way or not, don't think the c array looks the same as "decoded_img_string".
  */

  uint32_t width = root[kv.key()]["props"]["width"];
  uint32_t height = root[kv.key()]["props"]["height"];

  uint8_t decoded_img_string[width * height];
  const char* base64_string = root[kv.key()]["props"]["image"];
  unsigned int decode_int = decode_base64(base64_string, decoded_img_string);

  static lv_img_header_t my_img_header = {
  ...
  static lv_img_dsc_t my_img_dsc = {
    my_img_header,
    width * height * LV_COLOR_SIZE / 8,
    decoded_img_string,
  };

  lv_img_set_src(canvas_img, &my_img_dsc);
  lv_obj_set_pos(canvas_img, root[kv.key()]["props"]["pageX"], root[kv.key()]["props"]["pageY"]);
  lv_obj_set_size(canvas_img, width, height);
}
