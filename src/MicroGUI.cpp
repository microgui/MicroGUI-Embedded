...
void mgui_render_canvas_img(JsonPair kv, JsonObject root){
  ...
  static lv_color_t cbuf[LV_CANVAS_BUF_SIZE_TRUE_COLOR(CANVAS_WIDTH, CANVAS_HEIGHT)]; // What should the size be? Too large -> Can't build. Too small -> Crashes
  ...
  /*
  Everything below is an attempt at getting an image to render. At the moment it renders if set up correctly. 
  For this to work you need a "img_MicroGUI75.c" file, which is generated by the LVGL image converter: https://lvgl.io/tools/imageconverter
  This image can't be too large and needs to be modified from the genereated image converter. 
  */

  // Draw image on canvas
  lv_draw_img_dsc_t dsc;
  lv_draw_img_dsc_init(&dsc);
  LV_IMG_DECLARE(img_MicroGUI75);
  lv_canvas_draw_img(canvas_img, 0, 0, &img_MicroGUI75, &dsc);
}
